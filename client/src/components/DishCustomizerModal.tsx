...
import { Plus, Minus, ShoppingBag, Flame, Sparkles, Check, Utensils } from "lucide-react";
...
export function DishCustomizerModal({
  ...
  const [spiceLevel, setSpiceLevel] = useState("Boytag's Classic");
  ...
  const [extraDrink, setExtraDrink] = useState(false);
      ...
      <div className="space-y-5 max-h-[75vh] overflow-y-auto pr-1">
        ...
        <div className="flex items-center gap-4 rounded-2xl bg-cream p-3 border border-line">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="display text-base font-bold text-ink truncate">{product.name}</h3>
            <p className="text-xs text-muted line-clamp-1">{product.description}</p>
            <p className="display text-sm font-bold text-roast mt-0.5">{formatPeso(product.price)}</p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <label className="text-xs font-semibold uppercase tracking-wider text-muted">
            Spice & Marinade Blend:
          </label>
          <div className="grid grid-cols-3 gap-2 text-xs">
            {["Boytag's Classic", "Extra Garlic Lemongrass", "Labuyo Spicy Kick 🌶️"].map((s) => (
              <button
                key={s}
                type="button"
        ...
        <div className="space-y-2">
          <label className="text-xs font-semibold uppercase tracking-wider text-muted">
            Add-on Pairing Recommendations:
          </label>
          <div className="space-y-2">
            <label className="flex items-center justify-between rounded-xl border border-line bg-paper p-3 text-xs cursor-pointer hover:border-roast/40 transition">
              <div className="flex items-center gap-2">
                ...
                />
                <span className="font-semibold text-ink">Extra Fragrant Garlic Java Rice</span>
              </div>
            ...
            <label className="flex items-center justify-between rounded-xl border border-line bg-paper p-3 text-xs cursor-pointer hover:border-roast/40 transition">
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="h-4 w-4 rounded border-line text-roast accent-roast"
                />
                <span className="font-semibold text-ink">Extra House Sawsawan Trio Cup</span>
              </div>
            ...
            <label className="flex items-center justify-between rounded-xl border border-line bg-paper p-3 text-xs cursor-pointer hover:border-roast/40 transition">
              <div className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={extraAtchara}
                  onChange={(e) => setExtraAtchara(e.target.checked)}
                  className="h-4 w-4 rounded border-line text-roast accent-roast"
                />
                <span className="font-semibold text-ink">Special Pickled Papaya Atchara (200g)</span>
              </div>
              <span className="font-bold text-roast">+₱35.00</span>
            </label>
            ...
            <label className="flex items-center justify-between rounded-xl border border-line bg-paper p-3 text-xs cursor-pointer hover:border-roast/40 transition">
              <div className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={extraDrink}
                  onChange={(e) => setExtraDrink(e.target.checked)}
                  className="h-4 w-4 rounded border-line text-roast accent-roast"
                />
                <span className="font-semibold text-ink">Fresh Native Iced Calamansi with Honey</span>
              </div>
              <span className="font-bold text-roast">+₱45.00</span>
            </label>
          </div>
        </div>

        {/* Special Instructions */}
        <Field label="Kitchen Preparation Notes (Optional)">
          <input
            type="text"
            placeholder="e.g. Extra toasted skin, sauce on the side, extra calamansi..."
            value={specialNotes}
        ...
        <div className="flex items-center justify-between border-t border-line pt-4">
          <div className="flex items-center rounded-2xl border border-line bg-cream p-1">
            <button
              type="button"
              onClick={() => {
                sound.play("click");
              ...
              disabled={quantity <= 1}
              className="grid h-8 w-8 place-items-center rounded-xl hover:bg-paper transition disabled:opacity-40"
            ...
            <button
              ...
              onClick={() => {
                sound.play("click");
                setQuantity((q) => Math.min(product.availableQty, q + 1));
              }}
          ...
          <div className="flex items-center gap-4">
            <div className="text-right">
              <p className="text-[11px] uppercase tracking-wider text-muted font-bold">Total</p>
              <p className="display text-xl font-bold text-roast">{formatPeso(totalPrice)}</p>
            </div>
